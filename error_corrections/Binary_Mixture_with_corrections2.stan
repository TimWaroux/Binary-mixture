...
model {
  ...
  for(d in 1:n_days){
    beta_scale[d] ~ normal( 0, 3 ); // prior distributions of scale vars
    beta_raw[d,] ~ normal( 0, 1 );
  } 
  delta ~ normal(2, 1);
  p ~ beta(2,2);
  ...
  logod_ctrl_neg ~ normal( mu_hat_plate[1:ctrls_neg],
                          sqrt(square(sigma1)+square(sigma_plate)));
  logod_ctrl_pos ~ normal( mu_hat_plate[ctrls_neg+1:ctrls_neg+ctrls_pos]+delta,
                          sqrt(square(sigma2)+square(sigma_plate)));
